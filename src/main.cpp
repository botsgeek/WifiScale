...
std::string SSID = "DAVID";
std::string password = "Olakunle001/";
...
//     while (true) {
//         float weight;
////        if (xQueueReceive(weightQueue, &weight, portMAX_DELAY) == pdTRUE) {
////            awsManager.publishMessage(weight);
////        }
//          if (xQueueReceive(weightQueue, &weight, pdMS_TO_TICKS(100)) == pdTRUE) {
//             awsManager.publishMessage(weight);
//          }
//    }
//}
...
//         DBErrors extractErr = db_Handler.extractIDs(weight, ids);
//         if (extractErr == DBErrors::OK) {
...
//           deleteBool = true;
//         } else {
//           Serial.printf("Error extracting IDs: %d\n", (int)extractErr);
//         }
...
//}
void publishTask(void* parameter) {//global instance
  while (true)
  {
    // No need to pass weight to publishMessage anymore
    // awsManager.publishMessage();
    do{
      std::string weight;
      DBErrors err = db_Handler.read_DB_Multiple(5, weight);
    if (err == DBErrors::OK)
    {
      Serial.printf("latest db row is: %s\n", weight.c_str());
      ...
      //Publish to aws
      //check if your publish is successful befor delete
      //awsManager.publishMessage();
       if(err == DBErrors::OK) {
  ...
  ////            awsManager.publishMessage(weight);
  ////        }
  //        // No need for weight, as we're publishing a fixed message
  //        awsManager.publishMessage();
  ...
  void setup()
  {
    ...
    //timeQueue = xQueueCreate(QUEUE_SIZE, sizeof(std::string));
    // Connect to AWS once WiFi is established
    // awsManager.connectAWS();

    sdCardManager.initSDCard();
    // AsyncElegantOTA.begin(&server);
    // myServer.begin(server);
    ...
    // Timing.printLocalTime(currentTimestamp);
    Serial.printf("\nHeap size: %d\n", ESP.getHeapSize());
    ...
    aws_err = awsManager.publish(publish_topic,dummy);
    while(aws_err!= AWSErrors::OK){
    ...
    //xTaskCreatePinnedToCore(connectivityTask, "connectivityTask Task", 16384, NULL, 1, NULL, PRO_CPU_NUM);
  }
